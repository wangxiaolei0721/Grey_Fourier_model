...
for i=1:dev_length % error deviation
    ...
    for order=0:order_upper % candidate order
        for k=1:rep  % repetitions
            x_pre_val(:,k)=GFM_linear_integral(x_sim_train(:,k),omega,order,val);
        end
        %% mean absolute percetage error
        ape=abs((x_pre_val-x_train_val)./x_train_val)*100;
        ape_fit=ape(1:train,:);
    ...
    if i==1 % ylabel
        text(0,0,['log_{10}(MAPE)'],'Rotation',90,'FontName','Book Antiqua','FontSize',12)
    end
    set(gca,'FontName','Book Antiqua','FontSize',12 ,'Ylim',[-1.2,1.7])
end
%% save figure
